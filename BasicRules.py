...
class KingRules(object):
    ...
    def get_reached_spots(spot: Spot, board: Board):
        ...
        for spot in castling_spots:
            reached_spots.append(spot)
        return reached_spots
    ...
    def _get_castling_spots(spot: Spot, board: Board):
        ...
        p1, p2, p3, p4, p5 = board.board[0][1], board.board[0][2], board.board[0][4], board.board[0][5], board.board[0][6]
        if isinstance(left, Rook):
            if not left.is_moved and isinstance(p1, numbers.Number) and isinstance(p2, numbers.Number):
                castling_spots.append(Spot(row, 1))
        ...
        if isinstance(right, Rook):
            if not right.is_moved:
                if isinstance(p3, numbers.Number) and isinstance(p4, numbers.Number) and isinstance(p5, numbers.Number):
                    castling_spots.append(Spot(row, 5))
...
class PawnRules(object):
    ...
    def get_influence_spots(spot: Spot, board: Board):
        ...
        if pawn.team == 0:
            ...
            if row - 1 >= 0 and col - 1 >= 0:
                influenceSpots.append(Spot(row - 1, col - 1))
    ...
    def get_reached_spots(spot: Spot, board: Board):
        ...
        if pawn.team == 0:
            if row + 1 <= 7:
                if isinstance(board.board[row + 1][col], numbers.Number):
                    reached_spots.append(Spot(row + 1, col))

            if row == 1:
                if isinstance(board.board[row + 1][col], numbers.Number) and isinstance(board.board[row + 2][col], numbers.Number):
                    reached_spots.append(Spot(row + 2, col))
        ...
        if end.row != -1 and end.col != -1:
            if isinstance(board.board[end.row][end.col], Pawn) and abs(end.row - start.row) == 2:
                if abs(end.col - col) == 1:
                    if pawn.team == 0 and row == 4:
                        reached_spots.append(Spot(row + 1, end.col))
...
class BasicRules(object):
    ...
    @staticmethod
    def get_influence_spots(spot: Spot, board: Board):
        rule = chooseRules(board.board[spot.row][spot.col])
